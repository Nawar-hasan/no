...
from .models import db, Service, ServicePricing

@app.route('/admin/pricing')
def admin_pricing():
    services = Service.query.all()
    pricing_list = ServicePricing.query.join(Service).all()
    return render_template('admin_pricing.html', services=services, pricing_list=pricing_list)
...
def add_pricing():
    ...
    new_pricing = ServicePricing(service_id=service_id, price=price, currency=currency)
    db.session.add(new_pricing)
    db.session.commit()
    return redirect(url_for('admin_pricing'))

@app.route('/admin/pricing/edit', methods=['POST'])
def edit_pricing():
    pricing_id = request.form.get('pricing_id')
    pricing = ServicePricing.query.filter_by(id=pricing_id).first()
    pricing.price = float(request.form.get('price'))
    pricing.currency = request.form.get('currency')
    db.session.commit()
    return redirect(url_for('admin_pricing'))

@app.route('/admin/pricing/delete', methods=['POST'])
def delete_pricing():
    pricing_id = request.form.get('pricing_id')
    pricing = ServicePricing.query.filter_by(id=pricing_id).first()
...
from .models import db, Service, User, Transaction, ServicePricing
...
def request_service():
    if request.method == 'POST':
        ...
        user_id = request.form.get('user_id')  # This should be obtained from the session or login system
        
        # Fetch service pricing
        pricing = ServicePricing.query.filter_by(service_id=service_id).first()
        if not pricing:
        ...
        if payment_type == 'points':
            # Example logic to check if user has enough points (needs actual implementation)
            user = User.query.filter_by(id=user_id).first()
            if user.points < pricing.price:
                flash('Not enough points.', 'error')
                return redirect(url_for('request_service'))
            # Deduct points here
            user.points -= pricing.price
        elif payment_type == 'bank':
            # Implement bank transfer logic or redirect to bank payment gateway
            pass

        # Create a new transaction
        new_transaction = Transaction(user_id=user_id, service_id=service_id, amount=pricing.price, type=payment_type)
        db.session.add(new_transaction)
        db.session.commit()

        flash('Service requested successfully.', 'success')
        return redirect(url_for('my_services'))  # Redirect to a page showing user's services

    services = Service.query.all()
    return render_template('service_request.html', services=services)

@app.route('/admin/subscribers')
def admin_subscribers():
    subscribers = User.query.filter_by(is_admin=False).all()  # Assuming non-admins are regular subscribers
...
def update_order_status():
    ...
    new_status = request.form.get('status')  # Assuming status is passed as a string, e.g., "Completed"
    order = Order.query.filter_by(id=order_id).first()
    if order:
        order.is_done = (new_status == 'Completed')
        db.session.commit()
...
@app.route('/admin/orders/send_certificate', methods=['POST'])
def send_certificate():
    order_id = request.form.get('order_id')
    order = Order.query.filter_by(id=order_id).first()
    if order and order.is_done:
        # Logic to send the certificate via email or provide a download link
        flash('Certificate sent to customer.', 'success')
    else:
        flash('Order is not completed yet or not found.', 'error')
    return redirect(url_for('admin_orders'))
...
def register():
    if request.method == 'POST':
        ...
        name = request.form['name']
        password = request.form['password']
        # Hash the password: (Assuming hashing is needed)
        hashed_password = generate_password_hash(password)
        
        new_user = User(email=email, name=name, password=hashed_password, email_verified=False)
        db.session.add(new_user)
        db.session.commit()
        ...
        msg = Message('Confirm Email', sender='your-email@example.com', recipients=[email])
        link = url_for('confirm_email', token=token, _external=True)
        msg.body = 'Your link is {}'.format(link)
        mail.send(msg)
        
        return '<h1>The email you entered is {}. Check your email to complete registration.</h1>'.format(email)
...
@app.route('/admin/design_certificate', methods=['GET', 'POST'])
def design_certificate():
    if request.method == 'POST':
        title = request.form.get('title')
        body = request.form.get('body')
        new_design = CertificateDesign(title=title, body=body)
        db.session.add(new_design)
        db.session.commit()
        flash('Certificate design saved successfully.', 'success')
        return redirect(url_for('design_certificate'))
    return render_template('certificate_design.html')

@app.route('/admin/designs')
def list_designs():
    designs = CertificateDesign.query.all()
    return render_template('list_designs.html', designs=designs)

@app.route('/admin/site_design', methods=['GET', 'POST'])
def admin_site_design():
    if request.method == 'POST':
        logo_url = request.form.get('logo_url')
        navbar_color = request.form.get('navbar_color')
        font_style = request.form.get('font_style')
        
        design_settings = SiteDesign.query.first()
        if not design_settings:
            design_settings = SiteDesign()
            db.session.add(design_settings)
        
        design_settings.logo_url = logo_url
        design_settings.navbar_color = navbar_color
        design_settings.font_style = font_style
        db.session.commit()
        
        flash('Design settings updated successfully.', 'success')
        return redirect(url_for('admin_site_design'))
    
    design_settings = SiteDesign.query.first()
    if not design_settings:
        design_settings = SiteDesign(logo_url='', navbar_color='#FFFFFF', font_style='Arial')
    
    return render_template('site_design.html', design_settings=design_settings)

@app.route('/my_orders')
def my_orders():
    user_id = 'example_user_id'  # This should be dynamically set based on logged-in user
    orders = Order.query.filter_by(user_id=user_id).all()
    return render_template('my_orders.html', orders=orders)

@app.route('/send_certificate/<int:order_id>')
def send_certificate(order_id):
    order = Order.query.get(order_id)
    if not order or not order.is_done:
        flash('Order is not completed or not found.', 'error')
        return redirect(url_for('my_orders'))
    
    # Simulate fetching a certificate URL or file path
    certificate_url = f"https://example.com/certificates/{order.id}.pdf"
    
    # Simulate sending an email with the certificate
    msg = Message("Your Certificate", sender="from@example.com", recipients=["to@example.com"])
    msg.body = f"Here is your certificate: {certificate_url}"
    mail.send(msg)
    
    # Update the order to reflect that the certificate was sent
    order.certificate_sent = True
    db.session.commit()
    
    flash('Certificate sent to email.', 'success')
    return redirect(url_for('my_orders'))
